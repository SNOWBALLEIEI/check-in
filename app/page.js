...
function HouseCard({ house, attendance, onToggle, onSetAll, isOpen, onToggleOpen, onConfirm, isConfirmed, selectedType, onTypeChange }) {
    ...
    >
          ...
          <div className="flex items-center gap-3">
            <div>
              ...
              <p className="text-white/60 text-xs">
                {isConfirmed ? 'ยืนยันแล้ว' : `เช็คแล้ว ${checked}/${total} คน`}
              </p>
            </div>
          </div>
      ...
      >
        <div className={`${house.cardBg} px-4 pt-3 pb-2`}>
          {/* Type Selector */}
          <div className="mb-3">
            <p className="text-gray-500 text-xs mb-2">ประเภท Airdrop</p>
            <div className="flex gap-1.5">
              {AIRDROP_TYPES.map(type => (
                <button
                  key={type}
                  onClick={() => onTypeChange(house.id, type)}
          ...
          <div className="pb-3">
            <button
              ...
              className={`
                ...
                ${isConfirmed
                  ? 'bg-gray-800/60 text-gray-500 border border-gray-700/40 cursor-default'
                  : 'bg-gradient-to-r from-yellow-700 to-amber-600 hover:from-yellow-600 hover:to-amber-500 text-white shadow-lg shadow-yellow-700/25 hover:shadow-yellow-600/40'
                }
              `}
            >
              {isConfirmed ? (
                ...
                <>
                  <span>ยืนยันเช็คชื่อ {house.name}</span>
                </>
              )}
            </button>
          </div>
...
function HouseConfirmModal({ house, attendance, onClose, onConfirm, confirmed, selectedType }) {
  const hs = calcHouseStats(house.id, house.members, attendance)
  const checked = hs.present + hs.leave + hs.absent
  const total = house.members.length
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative bg-gray-900 border border-yellow-900/50 rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-scale-in">
        {confirmed ? (
          <div className="text-center py-10 animate-bounce-soft">
            <div className="text-2xl font-bold text-yellow-400 mb-2">บันทึกสำเร็จ!</div>
            <p className="text-gray-400 mt-2 text-sm">{house.name} ถูกบันทึกแล้ว</p>
          </div>
        ) : (
          <>
            {/* Title */}
            <div className="mb-5">
              <h3 className="text-lg font-bold text-white">ยืนยัน {house.name}</h3>
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-gray-500 text-xs">เช็คแล้ว {checked}/{total} คน</span>
              </div>
            </div>
            ...
            <div className="space-y-1.5 mb-5 max-h-44 overflow-y-auto pr-1">
              {house.members.map((member, idx) => {
                const status = attendance[getKey(house.id, idx)]
                const cfg = STATUSES.find(s => s.key === status)
                return (
                  <div key={idx} className="flex items-center justify-between bg-gray-800/40 rounded-lg px-3 py-1.5">
                    <span className="text-gray-300 text-xs">{member}</span>
                    {cfg ? (
                      <span className={`text-xs font-bold ${cfg.textColor}`}>{cfg.label}</span>
                    ) : (
                      <span className="text-gray-600 text-xs">ยังไม่เช็ค</span>
                    )}
                  </div>
                )
              })}
            ...
            {checked < total && (
              <div className="bg-yellow-500/10 border border-yellow-500/25 rounded-xl px-3 py-2.5 mb-4 flex items-center gap-2">
                ...
                <p className="text-yellow-300/80 text-xs">
                  ยังไม่ได้เช็ค <span className="font-bold text-yellow-300">{total - checked} คน</span>
                </p>
              </div>
            )}
...
export default function AttendancePage() {
  ...
  const [confirmedHouses, setConfirmedHouses] = useState(new Set())
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/houses`)
      ...
      .then(data => {
        ...
        setHouses(withStyle)
        setSelectedTypes(Object.fromEntries(withStyle.map(h => [h.id, AIRDROP_TYPES[0]])))
      })
  ...
  const handleTypeChange = (houseId, type) => {
    setSelectedTypes(prev => ({ ...prev, [houseId]: type }))
  }

  const handleOpenModal = (houseId) => {
    setModalConfirmed(false)
    setModalHouseId(houseId)
  }

  const handleFinalConfirm = async () => {
    const house = houses.find(h => h.id === modalHouseId)
    const hs = calcHouseStats(modalHouseId, house.members, attendance)
    const record = {
      id: `${modalHouseId}-${Date.now()}`,
      timestamp: new Date().toISOString(),
      houseId: modalHouseId,
      houseName: house.name,
      airdropType: selectedTypes[modalHouseId] || AIRDROP_TYPES[0],
      members: house.members.map((name, idx) => ({
        name,
        status: attendance[getKey(modalHouseId, idx)] || null,
      })),
      present: hs.present,
      leave: hs.leave,
      absent: hs.absent,
    ...
    setConfirmedHouses(prev => new Set([...prev, modalHouseId]))
    setTimeout(() => {
      setModalHouseId(null)
      setOpenHouses(prev => {
        const next = new Set(prev)
        next.delete(modalHouseId)
        return next
      })
    }, 1800)
  }

  const modalHouse = houses.find(h => h.id === modalHouseId)
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 mb-10 items-start">
          {housesLoading ? (
            ...
            <HouseCard
              ...
              isConfirmed={confirmedHouses.has(house.id)}
              selectedType={selectedTypes[house.id]}
      ...
      {/* ── Per-house Modal ── */}
      {modalHouseId && modalHouse && (
        <HouseConfirmModal
          house={modalHouse}
          attendance={attendance}
          confirmed={modalConfirmed}
          onClose={() => setModalHouseId(null)}
          onConfirm={handleFinalConfirm}
          selectedType={selectedTypes[modalHouseId]}
        />
      )}
    </div>
  )
