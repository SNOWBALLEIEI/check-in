...
const HOUSE_STYLE = {
  headerGradient: 'from-indigo-600 to-violet-500',
...
const PRACTICE_DAYS = ['วันพฤหัสบดี', 'วันศุกร์']

function getKey(houseId, idx) { return `${houseId}-${idx}` }
...
function calcHouseStats(houseId, members, attendance) {
  return {
    ...
    leaveProof: members.filter((_, i) => attendance[getKey(houseId, i)] === 'leave_proof').length,
...
function HouseCard({ house, attendance, onToggle, isOpen, onToggleOpen }) {
  const hs      = calcHouseStats(house.id, house.members, attendance)
  const checked = hs.present + hs.leave + hs.leaveProof + hs.absent
  const total   = house.members.length

  return (
    <div className={`${house.border} border rounded-3xl overflow-hidden shadow-xl transition-all duration-300`}>
      <button
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-white/60 text-xs">เช็คแล้ว {checked}/{total} คน</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex gap-1.5">
              <StatusBadge count={hs.present}    status="present"    />
              <StatusBadge count={hs.leave}      status="leave"      />
              <StatusBadge count={hs.leaveProof} status="leave_proof" />
      ...
      <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className={`${house.cardBg} px-4 pt-3 pb-4`}>
              ...
              return (
                <div key={idx} className="flex items-center gap-3">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <div className="w-6 h-6 rounded-lg bg-gray-800/80 border border-gray-700/50 flex items-center justify-center text-xs text-gray-500 font-semibold flex-shrink-0">
                      {idx + 1}
                    </div>
                  ...
                  <div className="flex gap-1.5 flex-shrink-0">
                    {STATUSES.map(s => (
                      <button
                        ...
                        className={`px-3 h-8 rounded-xl text-xs font-bold border transition-all duration-150 active:scale-95 ${current === s.key ? s.activeClass : s.inactiveClass}`}
                      >
                        {s.label}
                      </button>
                    ))}
...
function ConfirmModal({ houses, attendance, selectedDay, onClose, onConfirm, confirmed }) {
  const allStats    = houses.map(house => ({ house, stats: calcHouseStats(house.id, house.members, attendance) }))
  const total       = houses.reduce((s, h) => s + h.members.length, 0)
  const totalChecked = allStats.reduce((s, { stats }) => s + stats.present + stats.leave + stats.leaveProof + stats.absent, 0)

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/75 backdrop-blur-sm" onClick={!confirmed ? onClose : undefined} />
      <div className="relative bg-gray-900 border border-indigo-900/50 rounded-3xl p-6 w-full max-w-lg shadow-2xl">
        {confirmed ? (
          <div className="text-center py-10">
            <div className="text-2xl font-bold text-indigo-400 mb-2">บันทึกสำเร็จ!</div>
            <p className="text-gray-400 mt-2 text-sm">บันทึกการเช็คชื่อซ้อมทั้งหมดแล้ว</p>
          </div>
        ) : (
          <>
            <div className="mb-4">
              <h3 className="text-lg font-bold text-white">ยืนยันเช็คชื่อซ้อม</h3>
              <div className="flex items-center gap-2 mt-1">
                <span className="inline-block px-2.5 py-0.5 bg-indigo-600/20 border border-indigo-600/30 rounded-lg text-indigo-300 text-xs font-medium">{selectedDay}</span>
            ...
            <div className="space-y-2 mb-5 max-h-64 overflow-y-auto pr-1">
              {allStats.map(({ house, stats }) => (
                <div key={house.id} className="bg-gray-800/40 rounded-xl px-4 py-3 flex items-center justify-between">
                  <span className="text-indigo-300 text-sm font-bold">{house.name}</span>
                  <div className="flex gap-3 text-xs font-semibold">
                    <span className="text-emerald-400">{stats.present} มา</span>
                    <span className="text-amber-400">{stats.leave} ลา</span>
                    {stats.leaveProof > 0 && <span className="text-teal-400">{stats.leaveProof} ลา+หลักฐาน</span>}
            ...
            {totalChecked < total && (
              <div className="bg-yellow-500/10 border border-yellow-500/25 rounded-xl px-3 py-2.5 mb-4 flex items-center gap-2">
                ...
                <p className="text-yellow-300/80 text-xs">ยังไม่ได้เช็ค <span className="font-bold text-yellow-300">{total - totalChecked} คน</span></p>
              </div>
            )}

            <div className="flex gap-3">
              <button onClick={onClose} className="flex-1 py-3 bg-gray-800 hover:bg-gray-700/80 text-gray-300 rounded-xl font-medium text-sm transition-colors border border-gray-700/50">
                กลับไปแก้ไข
              </button>
              <button onClick={onConfirm} className="flex-1 py-3 bg-gradient-to-r from-indigo-700 to-violet-600 hover:from-indigo-600 hover:to-violet-500 text-white rounded-xl font-bold text-sm transition-all shadow-lg shadow-indigo-600/25 active:scale-95">
                ยืนยัน
              </button>
            </div>
          </>
...
export default function PracticePage() {
  ...
  const [selectedDay, setSelectedDay] = useState(PRACTICE_DAYS[0])
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/houses`)
      .then(r => r.json())
      .then(data => setHouses(data.map(h => ({ ...h, ...HOUSE_STYLE }))))
      .catch(err => console.error('โหลด houses ไม่สำเร็จ:', err))
      .finally(() => setHousesLoading(false))
  }, [])

  useEffect(() => {
    setDateStr(new Date().toLocaleDateString('th-TH', {
  ...
  const handleToggle = (houseId, idx, status) => {
    ...
    setAttendance(prev => ({ ...prev, [key]: prev[key] === status ? undefined : status }))
  }
  ...
  const handleConfirmAll = async () => {
    ...
    const records = houses.map(house => {
      const hs = calcHouseStats(house.id, house.members, attendance)
      return {
        id: `practice-${house.id}-${Date.now()}`,
        timestamp,
        houseId:     house.id,
        houseName:   house.name,
        practiceDay: selectedDay,
        members: house.members.map((name, idx) => ({
          name,
          status: attendance[getKey(house.id, idx)] || null,
        })),
        present: hs.present,
        leave:   hs.leave + hs.leaveProof,
        absent:  hs.absent,
      }
    })
    ...
    try {
      await Promise.all(records.map(record =>
        fetch(`${API_URL}/api/practice`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(record),
        }).then(r => { if (!r.ok) throw new Error('บันทึกไม่สำเร็จ') })
      ))
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="fixed inset-0 overflow-hidden pointer-events-none select-none" aria-hidden>
        <div className="absolute -top-48 -left-32 w-[500px] h-[500px] bg-indigo-700/15 rounded-full blur-3xl" />
        <div className="absolute top-0 right-0 w-96 h-96 bg-violet-800/10 rounded-full blur-3xl" />
        <div className="absolute bottom-0 left-1/3 w-[450px] h-[450px] bg-indigo-900/15 rounded-full blur-3xl" />
      </div>

      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8">

        <header className="text-center mb-10">
          <h1 className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-indigo-400 via-violet-300 to-indigo-400 bg-clip-text text-transparent leading-tight">
            เช็คชื่อซ้อม
          </h1>
        ...
        {/* Day Selector */}
        <div className="mb-6">
          <p className="text-gray-500 text-xs mb-2 text-center">วันซ้อม</p>
          <div className="flex gap-2 justify-center flex-wrap">
            {PRACTICE_DAYS.map(day => (
              <button
                key={day}
                onClick={() => setSelectedDay(day)}
                className={`px-6 py-2 text-sm font-medium rounded-xl border transition-all duration-150 ${
                  selectedDay === day
        ...
        {!housesLoading && houses.length > 0 && (
          <div className="sticky bottom-6 flex justify-center pb-2">
            <button
              ...
              className="px-10 py-4 bg-gradient-to-r from-indigo-700 to-violet-600 hover:from-indigo-600 hover:to-violet-500 text-white rounded-2xl font-bold text-base shadow-lg shadow-indigo-700/30 hover:shadow-indigo-600/50 transition-all duration-200 active:scale-95"
            >
              ยืนยันเช็คชื่อซ้อม
            </button>
          </div>
